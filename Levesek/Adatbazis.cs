...
namespace Levesek
{
    internal class Adatbazis
    {
        ...
        internal List<Levesek> getAllleves()
        {
            ...
            using (MySqlDataReader dr = sqlCommand.ExecuteReader())
            {
                ...
                while (dr.Read())
                {
                    Levesek leves = new Levesek(dr.GetString("megnevezes"), dr.GetDouble("kaloria"), dr.GetDouble("feherje"), dr.GetDouble("zsir"), dr.GetDouble("szenhidrat"), dr.GetDouble("hamu"), dr.GetDouble("rost"));
                    levesek.Add(leves);
                }
                return levesek;
            }
